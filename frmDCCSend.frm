...
Public lngFileStart     As Long
Public strFullPath  As String
Public intPacketSize    As Integer

Dim lngStartTime As Long
Dim lngCurTime As Long
Dim dblLastPer As Double
Function GetByteChunk(strFile As String, lngStart As Long) As String
    Dim strInput As String, lngLen As Long, lngFsize As Long
    lngLen = intPacketSize
    On Error Resume Next
    Close #1
    Open strFile For Binary As #1
        lngFsize = LOF(1)
        If lngStart >= lngFsize Then GetByteChunk = "": Exit Function
        If lngStart + lngLen > lngFsize Then lngLen = lngFsize - lngStart
        strInput = String(lngLen, 0)
        Get #1, lngStart + 1, strInput
    Close #1
    GetByteChunk = strInput
    strInput = ""
End Function
...
Public Sub UpdateInfo()
    Dim strTimeElapsed As String, lngLeft As Long, strPerc As String
    ...
    On Error Resume Next
    lngBps = ((lngSentRcvd - lngFileStart) / (lngTimeElapsed / 1000))
    ...
    Dim lngPerWidth As Long, dblPercent As Double, drawWidth As Long
    dblPercent = Format(lngSentRcvd / lngFileSize * 100, "###.##")
    strPerc = dblPercent
    If Len(RightOf(strPerc, ".")) = 1 Then strPerc = strPerc & "0"
    If InStr(strPerc, ".") = 0 Then strPerc = strPerc & ".00"
    Me.Caption = "DCC " & lblType & " - " & strPerc & "%"
    
    If dblPercent <> dblLastPer Then
        Client.DrawToolbar
        dblLastPer = dblPercent
    End If
...
Private Sub sock_ConnectionRequest(ByVal requestID As Long)
    ...
    sock.SendData GetByteChunk(strFullPath, lngSentRcvd)
    lngSentRcvd = lngSentRcvd + intPacketSize
    lngStartTime = GetTickCount
End Sub
...
Private Sub sock_DataArrival(ByVal bytesTotal As Long)
    If lblType = "Receive" Then
        ...
        On Error Resume Next
    Else
        If lngSentRcvd >= FileLen(strFullPath) Then
            lngSentRcvd = FileLen(strFullPath)
            lblStat = "File sent"
            UpdateInfo
            sock.Close
            Exit Sub
